...
{{-- {{ dd($product->bundle_options()->with(['product', 'bundle_option_products', 'bundle_option_products.product'])->get()) }} --}}
    ...
    <script type="text/x-template" id="bundle-option-item-template">
        <accordian :active="true">
            ...
            <div slot="body">
                ...
                <div class="control-group" :class="[errors.has(inputName + '[max_count]') ? 'has-error' : '']">
                    <label class="required">{{ __('admin::app.catalog.products.max_count') }}</label>

                    <input type="text" v-validate="'required|numeric|min_value:0'" :name="inputName + '[max_count]'" v-model="option.max_count" class="control" data-vv-as="&quot;{{ __('admin::app.catalog.products.max_count') }}&quot;"/>

                    <span class="control-error" v-if="errors.has(inputName + '[max_count]')">@{{ errors.first(inputName + '[max_count]') }}</span>
                </div>
