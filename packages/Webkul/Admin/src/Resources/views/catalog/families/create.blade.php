@php
    $locales =  app('Webkul\Core\Repositories\LocaleRepository')->all()->toJson();
@endphp
...
@section('content')
    <div class="content">
        <form method="POST" id="form_create" action="{{ route('admin.catalog.families.store') }}" @submit.prevent="onSubmit">
...
@push('scripts')
                    ...
                    <form method="POST" data-vv-scope="add-group-form" @submit.prevent="addGroup('add-group-form')">
                        ...
                        <div class="page-content">
                            <div class="form-container">
                                ...
                                <div v-for="translation in group.translations" class="control-group">
                                    <label :for="`locale-${translation.locale}-${translation.id}`">@{{ `Title [${translation.locale}]`}}</label>
                                    <input type="text" v-model="translation.title" class="control" :id="`locale-${translation.locale}-${translation.id}`" :name="`${translation.locale}[title]`" />
                                </div>
                    ...
                    <form method="POST" data-vv-scope="edit-group-form" @submit.prevent="updateGroup('edit-group-form')">
                        ...
                        <div class="page-content">
                            <div class="form-container">
                                ...
                                <div v-for="translation in editGroup.translations" class="control-group">
                                    <label :for="`locale-${translation.locale}-${translation.id}-${translation.attribute_group_id}`">@{{ `Title [${translation.locale}]`}}</label>
                                    <input type="text" v-model="translation.title" class="control" :id="`locale-${translation.locale}-${translation.id}-${translation.attribute_group_id}`" name="`${translation.locale}[title]`" />
                                </div>
    ...
    <script type="text/x-template" id="group-item-template">
        <accordian :title="group.name" :active="true">
            ...
            <div slot="body">
                ...
                <input  type="hidden" v-for="locale in group.translations" :name="translatedTitle(locale.locale)" :value="locale.title"/>
    ...
    <script>
        ...
        Vue.component('group-list', {
            ...
            created: function () {
                const locals_title = {!!$locales!!}
                let da = this

                // Initial Value
                locals_title.map(locale=>{
                    locale['title'] = ''
                    locale['locale'] = locale.code
                    da.group.translations.push(locale);
                    ...
                    this.$validator.validateAll(formScope).then(function (result) {
                        if (result) {
                            ...
                            if (filteredGroups.length) {
                                ...
                                this.group = {'name': '', 'position': '', 'is_user_defined': 1, 'custom_attributes': [] , 'translations':[]};
                                // const locals_title = {!!$locales!!}
                                //     let da = this

                                // locals_title.map(locale=>{

                                //     da.group[`${locale.code}[title]`] = '';
                    ...
                    this.$validator.validateAll(formScope).then(function (result) {
                        if (result) {
                            ...
                            if (filteredGroups.length > 1) {
                                ...
                                this.editGroup = {'name': '', 'position': '', 'is_user_defined': 1, 'translations':[], 'custom_attributes': []};
                                // const locals_title = {!!$locales!!}
                                //     let da = this

                                // locals_title.map(locale=>{

                                //     da.editGroup[`${locale.code}[title]`] = '';
        ...
        Vue.component('group-item', {
            ...
            methods: {
                ...
                translatedTitle(locale){
                    return `${this.groupInputName}[${locale}][title]`
                }
