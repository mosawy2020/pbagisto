...
@section('content')
    <div class="content">
        ...
        <form method="POST"  enctype="multipart/form-data" id="page-form" action="" @submit.prevent="onSubmit">
                ...
                <div class="form-container">
                    ...
                    <accordian title="{{ __('admin::app.cms.pages.general') }}" :active="true">
                        <div slot="body">
                            ...
                            <div class="control-group" :class="[errors.has('{{$locale}}[content_title]') ? 'has-error' : '']">
                                <label for="content_title">{{ __('admin::app.cms.pages.content_title') }}</label>

                                <input type="text" class="control" name="{{$locale}}[content_title]"  value="{{ old($locale)['content_title'] ?? ($page->translate($locale)['content_title'] ?? '') }}" data-vv-as="&quot;{{ __('admin::app.cms.pages.page-title') }}&quot;">

                                <span class="control-error" v-if="errors.has('{{$locale}}[content_title]')">@{{ errors.first('{!!$locale!!}[content_title]') }}</span>
                            </div>

                            <div class="control-group">
                                <label style="width:100%;">
                                    {{ __('admin::app.cms.pages.image') }}
                                    <span class="locale"></span>
                                </label>
                                <image-single remove-button-label="{{ __('admin::app.catalog.products.add-image-btn-title') }}"
                                    input-name="image"
                                    :image="{ 'id': '1', 'url': '{{ $page->image ? Storage::url($page->image) : '' }}' }"
                    ...
                    <accordian title="{{ __('admin::app.cms.pages.tabs') }}" :active="true">
                        <div slot="body">
                            
                            <faq-wrapper :faq-items="{{isset ($page->translate($locale)->faqs) ? $page->translate($locale)->faqs : '[]' }}"    input-name="{{$locale}}[faqs]"  :multiple="true" ></faq-wrapper>
                        </div>
                    </accordian>
