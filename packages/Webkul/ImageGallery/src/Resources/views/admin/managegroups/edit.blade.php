                ...
                <div class="control-group">
                    <select class="control" id="locale-switcher" onChange="window.location.href = this.value">
                        @foreach (core()->getAllLocales() as $localeModel)
                            ...
                            <option value="{{ route('imagegallery.admin.managegroups.edit', $category->id) . '?locale=' . $localeModel->code }}" {{ ($localeModel->code) == $locale ? 'selected' : '' }}>
                                {{ $localeModel->name }}
                            </option>
                    ...
                    <accordian :title="'{{ __('imagegallery::app.manage_group.edit.accordian_title') }}'" :active="true">
                        <div slot="body">
                            ...
                            <div class="control-group">
                                ...
                                <textarea class="control enable-wysiwyg" id="{{$locale }}[banner_text]" name="{{$locale }}[banner_text]">
                                    {{ old('group_banner_text') ?:          $category->translate(request()->query('locale'))->banner_text   }}
                                </textarea>
                            </div>

                            <div class="control-group" :class="[errors.has('banner') ? 'has-error' : '']">
                                <label for="banner" class="required">{{ __('imagegallery::app.images.edit.image') }}</label>
                                <image-single remove-button-label="{{ __('admin::app.catalog.products.add-image-btn-title') }}"
                                    input-name="banner"
                                    id="group_banner"
                                    :image="{ 'id': '1', 'url': '{{ Storage::url($category->banner) }}' }"
                                    :multiple="false"></image-single>
                                <span class="control-error" v-if="errors.has('group_banner')">@{{ errors.first('group_banner') }}</span>
                            </div>
