...
class ConfigurationController extends Controller
{
    ...
    //    $d=json_decode($velocityMetaData->advertisement, true);
    //     dd($d[1][4]);
        ...
        $velocityMetaData->advertisment_texts=json_decode($velocityMetaData->advertisment_texts, true) ?: [];
...
//dd($params);
        ...
        if(isset($params['home_page_apps']['image_1']) && $params['home_page_apps']['image_1']!=null)

        {
          //  dd("f");

            $path=    $this->uploadImage($params['home_page_apps']['image_1'],json_decode($velocityMetaData->home_page_apps,true),'velocity/app_section');

       //dd($d);

     $params['home_page_apps']['image_1']=$path;
        }else
        {
            $image=json_decode($velocityMetaData?->home_page_apps,true);
 if(isset($image['image_1']) && $image['image_1']!=null)

 $params['home_page_apps']['image_1']=$image['image_1'];
...
if(isset($params['social_links']))
{
    ...
    $params['subscription_bar_content']=json_encode($params['social_links']);
    }
        ...
        if (isset($params['images'])) {
            foreach ($params['images'] as $index => $images) {
              ...
              {  $params['advertisment_texts'][$index] =$params['textarea'][$index];
              }
       ...
       // dd( $params['advertisment_texts']);

            if( isset($params['textarea']))

            {  $params['advertisment_texts']=json_encode($params['advertisment_texts']);
            ...
            if(isset($params['home_page_apps']))
            {
                $params['home_page_apps']=json_encode($params['home_page_apps']);
            }

if(isset($params['footer_first_section']))
{

    $params['footer_first_section']=json_encode($params['footer_first_section']);
}



if(isset($params['footer_second_section']))
{

    $params['footer_second_section']=json_encode($params['footer_second_section']);
}


if(isset($params['footer_third_section']))
{

    $params['footer_third_section']=json_encode($params['footer_third_section']);
}



if(isset($params['footer_logo']) && $params['footer_logo']!=null)

{
  //  dd("f");

    $path=    $this->uploadImage($params['footer_logo'],json_decode($velocityMetaData?->footer_logo,true),'velocity/app_section');

//dd($d);

$params['footer_logo']=$path;
}else
{
    $image=json_decode($velocityMetaData?->footer_logo,true);
if(isset($image) && $image!=null)

$params['footer_logo']=$image;
...
if(isset($params['usage_items']))
{

$items=$params['usage_items'];

//dd($params['usage_items']);
foreach($items as $index=>$item)
{
  ...
  if($velocityMetaData?->usage_items)
 //  $data['brands'][$index] = $this->uploadImage($image,null,'gallary');

{

//dd($params['usage_items']);
    $data=json_decode($velocityMetaData?->usage_items,true);
    //dd($index)
   // dd($data);
    if(isset($data[$index]['image']) && $data[$index]['image']!=null)
    {

       // dd('h');
        if($params['usage_items'][$index]['image']==null)

        $params['usage_items'][$index]['image']=$data[$index]['image'];
        else

     $params['usage_items'][$index]['image']=$this->uploadImage($item['image'],null,'velocity/usage');
...
//dd($params['usage_items'][$index]['image']);
...
$params['usage_items']=json_encode($params['usage_items']);



}else
unset($params['usage_items']);


        unset($params['images']);
        unset($params['slides']);
      unset($params['textarea']);
      unset($params['social_links']);
      unset($params['usage_link']);
